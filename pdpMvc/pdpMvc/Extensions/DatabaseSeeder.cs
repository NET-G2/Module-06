    ...
    public class DatabaseSeeder
    {
        ...
        static void CreateStudents(PdpDbContext context)
        {
            ...
            for (int i = 0; i < 500; i++)
            {
                var student = new Student()
                {
                    ...
                    Balance = _faker.Random.Decimal(0, 10_000_000),
                };
            ...
            foreach (var student in students)
            {
                ...
                for (int i = 0; i < numberOfGroups; i++)
                {
                    ...
                    var enrollment = new Enrollment()
                    {
                        Student = student,
                        Group = group,
                        CourseStatus = pdpMvc.Models.Enums.CourseStatus.Active
                    };
        ...
        static void CreateModules(PdpDbContext context)
        {
            ...
            foreach (var subject in subjects)
            {
                for(int i=1; i<=10; i++)
                {
                    context.Modules.Add(new pdpMvc.Models.Module()
                    {
        ...
        static void CreateModuleTopics(PdpDbContext context)
        {
            ...
            foreach (var module in modules)
            {
                for (int i = 1; i <= 12; i++)
                {
                    context.ModuleTopics.Add(new pdpMvc.Models.ModuleTopic() 
                    {
                        ModuleId = module.Id,
                        Name = _faker.Lorem.Sentence(1),
                        Descrtiption = _faker.Lorem.Sentences(),
                    });
                }
            }
            context.SaveChanges();
